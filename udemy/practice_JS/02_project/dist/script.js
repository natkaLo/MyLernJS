...
window.addEventListener("DOMContentLoaded", function () {
  ...
  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["default"])('[name="phone"]'); //передаем селектор по аттрибуту

  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name = "name"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name = "message"]'); //показ скрытых в верстке карточек
  // showMoreStyles('.button-styles', '.styles-2');
  // показ карточек, загруженных из db.json через JSON сервер

  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '#styles .row'); // вставляем в элемент с классом row, который нах-ся в элементе с id style
...
var burger = function burger(menuSelector, burgerSelector) {
  var menuElement = document.querySelector(menuSelector),
      burgerElement = document.querySelector(burgerSelector); //чтобы изначально меню точно было скрыто
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('assets/dbCalc.json') //без JSON сервера. Просто к файлу
    .then(function (res) {
      // console.log(res); //получили в консоль массив данных (JSON сервер)
      //console.log(res.calc);//без JSON сервера. Просто к файлу
      var resValue = res.calc[0];
      var selectedIndex = sizeBlock.options.selectedIndex;
      var sizeBlockValue = resValue.sizeValues[selectedIndex]; // console.log(sizeBlockValue);

      selectedIndex = materialBlock.options.selectedIndex;
      var materialBlockValue = resValue.materialValues[selectedIndex];
      selectedIndex = optionsBlock.options.selectedIndex;
      var optionsBlockVkalue = resValue.optionsValues[selectedIndex]; //обязательно должны быть заполнены два поля (size и material). В верстке мы оставили пустой value для Выберите размер картины и Выберите материал картины
      ...
      if (sizeBlockValue == '' || materialBlockValue == '') {
        ...
        if (promocodeBlock.value === resValue.promocode) {
          //IWANTPOPART - промокод от заказчика
          resultBlock.textContent = Math.round(sum * 0.7);
        } else {
          resultBlock.textContent = sum;
        }
...
var filter = function filter() {
  ...
  var typeFilter = function typeFilter(markType, markClassName) {
    if (markClassName == prevActiveClassName) {
      return;
    } //скрываем все элементы


    var marPrevious = wrapper.querySelectorAll("." + prevActiveClassName);
    marPrevious.forEach(function (mark) {
      return hideElement(mark);
    }); // элемент с классом portfolio-no не находиться в all. Поэтому скроем его отдельно

    hideElement(no); //покажем элементы, которые находяться в переданном псевдомассива

    if (markType.length) {
      //если не undefinedZ
      markType.forEach(function (mark) {
        return showElement(mark);
      });
    } else {
      showElement(no);
    }
  ...
  var onFilter = function onFilter(className) {
    var mark = wrapper.querySelectorAll("." + className);
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var arr = item.files[0].name.split('.'); //разрежем имя файла(получим имя и расширение)
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (event) {
      event.preventDefault(); //создадим на форме оповещение пользователя
      ...
      statusMessage.appendChild(textMessage); // собираем данные, которые введены на форме. При помощи объекта FormData

      var formData = new FormData(item); //  //определимся, по какому адресу отправляем данные. Проверим у form (она у нас item) есть ли у нее родитель ( c классом popup-design)
      //для формы калькулятора в index.html добавили класс calc_form

      var api = item.closest('.popup-design') || item.classList.contains('calc_form') ? path.designer : path.question;
      console.log(api); // устанавливаем форме сумму в дата аттрибут в calc js
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) //возвращается промис (res в текстовом виде)
      .then(function (res) {
        console.log(res);
        ...
        clearInputs(); //удалим оповещение пользователя
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        //(статичное на основе матрицы)получаем все не цифры, которые есть в matrix и заменяем на пустое место
  ...
  var inputs = document.querySelectorAll(selector); //получим все инпуты переданные сюда из main.js
...
var modals = function modals() {
  ...
  function onModal(modal) {
    ...
    if (bShow) {
      modal.style.display = "block"; //чтобы body под модальным окном не скролировался
      //document.body.style.overflow = "hidden";
      //класс из будстрепа

      document.body.classList.add('modal-open');
      document.body.style.marginRight = "".concat(scrollWidth, "px"); //чтобы при появлении модального окна body документа не прыгало
    } else {
      modal.style.display = "none"; //чтобы body  скролировался
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; //в лог. контексте false
  ...
  function calcScroll() {
    ...
    var scrollWidth = div.offsetWidth - div.clientWidth; //div.offsetWidth - полная ширина,  div.clientWidth - ширина без прокрутки
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    var img = block.querySelector('img'); //все названия картинок у нас в формате (name.png). Нужно добавить -1
  ...
  function hideImg(block) {
    var img = block.querySelector('img'); //все названия картинок у нас в формате (name.png). Нужно убрать -1
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); //в index.html закоментировали карточки

  btn.addEventListener('click', function () {
    // getResource( 'http://localhost:3000/styles') //  http://localhost:3000/styles копируем из терминала после запуска JSON сервера (Resources)
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('assets/db.json') //без JSON сервера. Просто к файлу
    .then(function (res) {
      // console.log(res); //получили в консоль массив данных (JSON сервер)
